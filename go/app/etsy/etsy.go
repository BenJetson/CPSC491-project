...
func (c *Client) Search(
	...
	if res.StatusCode != http.StatusOK {
		return nil, errors.Errorf(
			"received non-OK status for listing request (%d)", res.StatusCode)
	}

	var etsyProducts []etsyProduct

	d := json.NewDecoder(res.Body)
	if d.Decode(&etsyProducts) != nil {
		return nil, errors.Wrap(err, "failed to decode Etsy products")
	}

	products := make([]app.CommerceProduct, len(etsyProducts))
	for idx, ep := range etsyProducts {
...
func (c *Client) GetProductByID(
	...
	var params url.Values
	c.injectAuth(&params)
	// params.Set("listing_id", strconv.Itoa(productID))
	...
	if len(etsyProducts) != 1 {
		return app.CommerceProduct{}, errors.Errorf(
			"expected exactly one product but found %d", len(etsyProducts))
	}

	p, err := etsyProducts[0].toCommerceProduct()
	if err != nil {
